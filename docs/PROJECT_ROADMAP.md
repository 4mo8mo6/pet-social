...
- 已完成：`2.5-R4` 为 Phase 2.5 增补最小可回归验证
- 已完成：`2.5-R4a` 建立 `web` 侧最小回归入口，覆盖社交候选状态分组、社交回合优先级、直接聊天提示三组共享逻辑
- 已完成：`2.5-R4b` 为后端好友门槛与社交回合选择补最小验证
...
- `2.5-R4a` 已完成最小前端回归校验：`web` 端新增 `npm run test:social`，当前已覆盖社交候选状态归类、分组顺序、社交回合优先级提示、会话门槛提示，并已通过 `test:social`、`eslint`、`build`
- `2.5-R4b` 已完成最小后端回归校验：`api/tests/test_pet_social_rules.py` 当前已覆盖“只有 accepted 好友才能直接聊天”“pending 请求的收口提示”“社交回合优先好友、其次 rejected/陌生对象、最后在全 pending 时阻塞”三组规则，并已通过 `python3 -m unittest discover -s tests -p 'test_*.py' -v` 与 `python3 -m compileall app tests`
...
- 已完成：`3-R1` 收口 `PetHomeScene` 的 scene 生命周期与初始化边界，修复 `/home` 首屏加载时对 `scene.events` 的过早访问
- 已完成：`3-R1` 机器自检通过：`web` 端 `npx eslint lib app` 与 `npm run build` 均已通过
- 已完成：`3-R2` 收口场景内点击宠物后的交互边界，明确“状态面板”和“聊天入口”的职责拆分
- 已完成：`3-R2` 机器自检通过：`web` 端 `npx eslint app lib` 与 `npm run build` 均已通过
- 已完成：`3-R3` 收口固定交互物件的反馈语义，明确“立即互动”和“仅作为行为目标点”的差异
- 已完成：`3-R3` 机器自检通过：`web` 端 `npx eslint app lib` 与 `npm run build` 均已通过
- 已完成：`3-R4` 为家庭场景增补最小前端回归验证，先覆盖固定物件语义与场景入口边界
...
- 已完成：`3-R7` 收口家庭场景提示的清理时机，为场景提示与面板提示补上统一自动清理策略，避免旧提示长期滞留
...
- 已完成：`3-R10` 收口家庭场景首次状态拉取失败时的空状态展示，区分“状态读取中”与“状态暂不可用”的降级文案，并统一 `/home` 与 `/my-pet` 的状态展示语义
- 已完成：`3-R10` 机器自检通过：`web` 端 `npm run test:home-scene`、`npm run test:social`、`npx eslint app lib tests` 与 `npm run build` 均已通过
- 已完成：`3-R11` 收口家庭场景首次加载与后续轮询的提示层级，让摘要文案只承担紧凑概览，空状态文案只在面板内承担详细说明，同步失败提示只保留给已有快照后的轮询降级
- 已完成：`3-R11` 机器自检通过：`web` 端 `npm run test:home-scene`、`npm run test:social`、`npx eslint app lib tests` 与 `npm run build` 均已通过
